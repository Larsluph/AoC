...
def detect_ssl(raw: str):
    ...
    # Detect in abba
    for i_start in range(len(data_in)-2):
        a,b,c = tuple(data_in[i_start:i_start+3])
        detected_in = detected_in or (a == d) and (b == c) and (a != b)

    # Detect out abba
    for i_start in range(len(data_out)-2):
        a,b,c = tuple(data_out[i_start:i_start+3])
