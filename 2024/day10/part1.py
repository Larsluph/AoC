def follow_trail(max_height: int, next_height: int, cursor: tuple[int, int], past_results: set[tuple[int, int]]):
    for x, y in ((-1, 0),
                 (0, -1),
                 (1, 0),
                 (0, 1)):
...
for j, row in enumerate(topo_map):
    for i, digit in enumerate(row):
        if digit == 0:
            ...
            temp_trails = set()
            follow_trail(9, 1, trail_head, temp_trails)
            print(len(temp_trails))
            for result in temp_trails:
                trails.add((trail_head, result))
