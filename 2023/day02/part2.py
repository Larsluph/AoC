...
for line in lines:
    ...
    red = max(map(lambda x: match_set(red_pattern, x), sets))
    green = max(map(lambda x: match_set(green_pattern, x), sets))
    blue = max(map(lambda x: match_set(blue_pattern, x), sets))

    pwrs.append(red * green * blue)
